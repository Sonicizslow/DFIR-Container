#!/bin/bash

# DFIR Browser Container startup script
# Ensures proper initialization and permissions for browser access

echo "Starting DFIR Browser Container..."

# Ensure proper permissions for dfiruser directories
chown dfiruser:dfiruser /home/dfiruser/.bashrc 2>/dev/null || true

# Ensure XRDP configuration has correct permissions
chown dfiruser:dfiruser /home/dfiruser/.xsession 2>/dev/null || true

# Create Desktop directory and ensure browser shortcut permissions
mkdir -p /home/dfiruser/Desktop
cat > /home/dfiruser/Desktop/epiphany.desktop << 'EOF'
[Desktop Entry]
Version=1.0
Name=Epiphany Web Browser
Comment=Web Browser for Link Investigation
Exec=epiphany
Icon=epiphany
Terminal=false
Type=Application
Categories=Network;WebBrowser;
StartupNotify=true
EOF
chmod +x /home/dfiruser/Desktop/epiphany.desktop
chown -R dfiruser:dfiruser /home/dfiruser/Desktop 2>/dev/null || true

echo "Browser container initialized successfully"

# Start supervisord
exec /usr/bin/supervisord -c /etc/supervisor/conf.d/supervisord.conf